...
In [VmDB](vmdb), we saw how the `VmDB` acts like a smart librarian, making sure the Ethereum Virtual Machine (EVM) reads the correct version of data when transactions run in parallel. It ensures the EVM gets the right information from the shared state managed by [Multi-Version Memory](mvmemory).
