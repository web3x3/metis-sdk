...
stateDiagram-v2
    [*] --> Locked: Lock METIS
    Locked --> Active: Mint NFT
    Active --> Waiting: Request Exit
    Waiting --> Unlocked: After 21 Days
    Unlocked --> [*]: Burn NFT

    Active --> UpdatedStake: Increase Stake
    UpdatedStake --> Active

    Active --> UpdatedSigner: Change Signer
...
If the user who has already locked the METIS needs to replace the signer, then the updateSigner method is provided in the LockingPool contract, which can help the user to replace the signer and transfer the NFT
