...
1. Hyperion Executor (including the OP-Node)
...
graph TB
    subgraph "Sequencer Node"
        HyperionExecutor[Hyperion Executor]
        Batcher[Batch Submitter]
        Adapter[Adapter Module]
        MPC[MPC Module]
    end

    Txs[Transactions] -->|Submit| HyperionExecutor
    HyperionExecutor -->|Sequence| Batcher
    Batcher -->|Request Signature| MPC
    MPC -->|Sign Batch| L1[Ethereum L1]

    PoS[PoS Layer] -->|Rotation Info| Adapter
    Adapter -->|Update| HyperionExecutor
```

## Hyperion Executor (including the OP-Node)
