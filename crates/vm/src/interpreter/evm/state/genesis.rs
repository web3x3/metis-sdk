...
pub fn empty_state_tree<DB: Blockstore>(_store: DB) -> anyhow::Result<State<DB>> {
    todo!()
}

/// Initially we can only set up an empty state tree.
/// Then we have to create the built-in actors' state that the Evm relies on.
/// Then we can instantiate an Evm execution engine, which we can use to construct FEVM based actors.
#[allow(unreachable_pub, dead_code)]
enum Stage<DB: Blockstore + 'static> {
    Tree(Box<State<DB>>),
    Exec(EvmExecState<DB>),
}

/// A state we create for the execution of genesis initialisation.
#[allow(unreachable_pub, dead_code)]
pub struct EvmGenesisState<DB>
where
    ...
    DB: Blockstore + Clone + 'static,
{
    pub async fn new(
        ...
        let state = Self {
            ...
            store,
            // multi_engine,
            stage: Stage::Tree(Box::from(state_tree)),
        };
    ...
    pub fn init_exec_state(
        ...
        self.stage = match self.stage {
            ...
            Stage::Tree(ref mut state_tree) => {
                ...
                let exec_state =
                    EvmExecState::new(self.store.clone(), &self.multi_engine, 1, params)
    ...
    pub fn exec_state(&mut self) -> Option<&mut EvmExecState<DB>> {
        match self.stage {
            Stage::Tree(_) => None,
